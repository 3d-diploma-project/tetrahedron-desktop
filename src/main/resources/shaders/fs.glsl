...
void main(void) {
  ...
  vec3 modelColorToDisplay;

  // blend between edge color and face color
  if (coloredInSelectedColor == 0) {
    modelColorToDisplay = vVertexIn.fragmentColor;
  } else {
    modelColorToDisplay = modelColor;
  }

  if (showElementMesh == 1) {
    color = vec4(mix(modelColorToDisplay, edgeColor, fEdgeIntensity), 1.0);
  } else {
    color = vec4(modelColorToDisplay, 1.0);
  }
}
