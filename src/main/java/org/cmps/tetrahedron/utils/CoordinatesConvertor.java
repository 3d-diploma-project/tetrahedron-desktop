...
public class CoordinatesConvertor {
    ...
    public Vector3f convertScreenToWorld(double mouseX, double mouseY) {
        Matrix4f viewProjMatrix = new Matrix4f(main.getProjMatrix()).mul(main.getViewMatrix());
        float[] depth = new float[1];
        GL11C.glReadPixels((int) mouseX, (int) (main.getHeight() - mouseY), 1, 1, GL11C.GL_DEPTH_COMPONENT, GL11C.GL_FLOAT, depth);

        return viewProjMatrix.unproject((float) mouseX,
                (float) (main.getHeight() - mouseY),
                depth[0],
                new int[] {0, 0, main.getWidth(), main.getHeight()},
                new Vector3f());
    }
    ...
    public void print3DCoordinates() {
        ...
        for (int i = 1; i < main.getVertices().size(); i++) {
            double xVertex = main.getVertices().get(i)[0];
            double yVertex = main.getVertices().get(i)[1];
            double zVertex = main.getVertices().get(i)[2];
